...
async function ccmPublicPost_(action,params={}){const response=await fetch(CCM_WEB_APP_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action,...params})});const data=await response.json();if(!response.ok)throw new Error(data?.message||data?.error||"CCM backend request failed.");return data;}
async function ccmExecute(action,params={}){if(!CCM_SESSION_TOKEN)throw new Error("CCM session has expired. Please sign in again.");const response=await fetch(CCM_WEB_APP_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"ccmExecute",sessionToken:CCM_SESSION_TOKEN,targetAction:action,params:params||{}})});const data=await response.json();console.log("CCM API ["+action+"]:",data);if(!response.ok)throw new Error(data?.message||data?.error||"CCM API request failed.");if(!data?.success)throw new Error(data?.message||data?.error||"CCM request was rejected.");return data;}
...
function openCCMApplication() {
     ...
     * Permissions are populated by authenticateCCMBackend().
     */
    applyNavigationPermissions_();
    wireAllCCMUI_();
